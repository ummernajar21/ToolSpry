// ============================================
// DATASET LOADER - SQL.js Initialization (FINAL)
// ============================================
...
// Wait for SQL.js <script> to be available
async function waitForSQLjsScript() {
  let attempts = 0;
...
// Initialize SQL.js and create empty DB
async function initSQLjs() {
  try {
    console.log('🔄 Waiting for SQL.js <script>...');
    await waitForSQLjsScript();

    console.log('🔄 Initializing SQL.js...');

    // ✅ LOCAL WASM PATH (NOT CDN)
    const locateFile = file => `/assets/wasm/${file}`;
    ...
    if (errorDiv && errorMsg) {
      ...
      errorMsg.textContent =
        'Failed to load SQL engine: ' + (error.message || error) + '. Please refresh the page.';
    }
...
function getCurrentDataset() {
  return currentDatasetName || 'employees';
}

function loadDataset(datasetName) {
  if (!isInitialized || !db) {
    console.error('❌ loadDataset called before database initialized');
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      console.warn('Warning dropping old tables:', e.message);
    }

    // Create table
    db.run(dataset.schema);

    // Insert data (single multi‑row INSERT string)
    db.run(dataset.data);
...
// Low-level: throws on error, returns raw SQL.js result array
...
// Optional wrapper if any old code still calls runQuery()
function runQuery(query) {
  try {
    const data = executeSQL(query);
